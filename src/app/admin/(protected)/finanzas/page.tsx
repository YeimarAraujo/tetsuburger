...
export default async function FinanzasPage({
  ...
  const [ordersRes, expensesRes, productionRes, closingsRes] = await Promise.all([
    supabase
      .from("orders")
      .select("id, status, total, delivery_fee, payment_method, origin, created_at")
      .gte("created_at", `${from}T00:00:00-05:00`)
      .lte("created_at", `${to}T23:59:59-05:00`),
    supabase
      .from("expenses")
      .select("id, expense_date, amount, category:expense_categories(name)")
  ...
  const orders = (ordersRes.data ?? []) as {
    id: string; status: string; total: string | number; delivery_fee: string | number;
    payment_method: string | null; origin: string; created_at: string;
  }[];

  const expenses = ((expensesRes.data ?? []) as unknown as {
    id: string; expense_date: string; amount: string | number;
